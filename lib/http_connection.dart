...
class HttpConnection implements IConnection {
  ...
  ITransport _constructTransport(HttpTransportType transport) {
    switch (transport) {
      case HttpTransportType.WebSockets:
        return WebSocketTransport(_accessTokenFactory, _logger, _options.logMessageContent);
      ...
      _transport!.onClose = (e) {
        ...
        if (features?.reconnect != null) {
          ...
          Future.microtask(() async {
            try {
              features?.disconnected?.call();
              await _transport!.connect(url, transferFormat);
              await features!.resend!();
            } catch (_) {
              callStop = true;
            }

            if (callStop) {
              _stopConnection(error: e);
            }
          });
        } else {
          _stopConnection(error: e);
  ...
  String _resolveNegotiateUrl(String url) {
    ...
    // Path negotiate ekleme
    var path = uri.path.endsWith("/")
        ? "${uri.path}negotiate"
        : "${uri.path}/negotiate";

    // query paramları kopyala
    final queryParams = Map<String, String>.from(uri.queryParameters);

    // negotiateVersion eklenmemişse ekle
    queryParams.putIfAbsent("negotiateVersion", () => _negotiateVersion.toString());

    // useStatefulReconnect kontrolü
    if (queryParams.containsKey("useStatefulReconnect")) {
      if (queryParams["useStatefulReconnect"] == "true") {
        _options.useStatefulReconnect = true;
      }
    } else if (_options.useStatefulReconnect == true) {
      queryParams["useStatefulReconnect"] = "true";
    }

    // yeni Uri oluştur
    final negotiateUri = uri.replace(path: path, queryParameters: queryParams);

    return negotiateUri.toString();
  }
